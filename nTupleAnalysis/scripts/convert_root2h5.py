...
variables = [variable("FvT"),
             ...
             # variable("nPVsGood"),
             variable("d01TruthMatch", dtype=np.int32),
             variable("d23TruthMatch", dtype=np.int32),
             variable("d02TruthMatch", dtype=np.int32),
             variable("d13TruthMatch", dtype=np.int32),
             variable("d03TruthMatch", dtype=np.int32),
...
def convert(inFile):
    ...
    for iEvtStart in range(0,iEvtEnd,chunkSize):
        ...
        for iEvt in range(iEvtStart, min(iEvtStart+chunkSize, iEvtEnd)):
            ...
            if (iEvt+1) % 1000 == 0 or iEvt+1 == iEvtEnd:
                ...
                rate = (iEvt+1)/elapsed
                secondsRemaining = (nEvts-iEvt)/rate
    ...
    store.close()
    elapsed = time.time()-start
    rate = nWritten/elapsed
    h, m, s = int(elapsed/3600), int(elapsed/60)%60, int(elapsed)%60

    print
    print " >> Wrote %10d events in %02d:%02d:%02d (%4.0f/s)"%(nWritten, h,m,s, rate)
